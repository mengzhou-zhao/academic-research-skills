...
|---|---|---|
| `thread.started` | `thread_id` (string, UUID) | `^[0-9a-f-]{36}$` |
| `turn.started` | (none additional) | |
| `item.completed` | `item.id`, `item.type`, `item.text` (when `item.type == "agent_message"`) | `item.text` non-empty for agent_message items |
...
- Exactly one `item.completed` event with `item.type == "agent_message"` carries the verdict text. The wrapper extracts this via `parse_audit_verdict.py` (§4.4 Step 4); the verdict text format is contracted by `shared/templates/codex_audit_multifile_template.md` Section 6 (severity-bucket count summary), NOT by this schema.
...
**Why these specific fields:** they are the events codex 0.125 actually emits, validated for shape. A forger needs to fabricate a UUID `thread_id`, the four-event ordering, `usage` integers on `turn.completed`, AND a syntactically-valid `item.completed.item.text` that passes `parse_audit_verdict.py` (which itself enforces audit-template Section 6 schema). Layer 2 alone does not guarantee genuineness, but it raises the floor — `echo '{}' >> fake.jsonl` no longer passes; a 4-event canonical sequence is the minimum forgery surface.

**`parse_audit_verdict.py` contract** (Phase 6.1 deliverable per §10): reads the JSONL stream, extracts the single `item.completed` event whose `item.type == "agent_message"`, and converts the audit-template Section 6 structured text into the `<run_id>.verdict.yaml` shape (§3.5). On parse failure (no agent_message event, malformed Section 6 text, etc.), emits AUDIT_FAILED verdict with `failure_reason: "JSONL parse error: <reason>"` per §4.6 case (b). The wrapper invokes parser with `--probe` first to validate the JSONL has parseable shape; absence of parseable shape triggers the AUDIT_FAILED branch in §4.4 Step 4.
...
| L2-3 | JSONL stream contains exactly one `thread.started` event with a UUID-formatted `thread_id` | Filter events by `type == "thread.started"`; assert single match; assert `thread_id` matches `^[0-9a-f-]{36}$` | §3.3 schema-level rule |
| L2-4 | JSONL stream contains exactly one `item.completed` event whose `item.type == "agent_message"` and whose `item.text` parses against the audit-template Section 6 verdict shape; stream ends with a `turn.completed` event carrying `usage` integers | Filter events by `type == "item.completed"` + `item.type == "agent_message"`, assert single match, hand `item.text` to `parse_audit_verdict.py --probe`; assert final event has `type == "turn.completed"` with `usage.input_tokens > 0` and `usage.output_tokens >= 0` | §3.3 schema-level rule |
...
3. **§5.6 phase identified.** The harness asserts the orchestrator exits at the §5.6 phase named in `expected_orchestrator_action.yaml.expected_phase` (e.g., `B10` for normal MATERIAL, `P-PB-gate` for ten-gate failure, `escalation` for round-3 MATERIAL).
...
**L3 — ARS_PASSPORT_RESET re-verification model.** Resolved in §5.5 as "re-run ten gating verification checks on resume; do not trust prior verification by passport-presence alone." Forward reference: §2.3 L3 → §5.5, R4 above. Status: **closed in §5.5.**
...
**Deliverables:** `scripts/run_codex_audit.sh` AND `scripts/parse_audit_verdict.py` (the JSONL→verdict parser the wrapper invokes per §4.4 Step 4 to extract the single `item.completed.item.text` agent_message event's text from the codex 0.125 event stream and convert it into the `<run_id>.verdict.yaml` file per §3.5). Both implement §4 contract end-to-end.
...
**Includes:**
- Per-row implementation of §3.7 families A (A1–A6), B (B1–B10), C (C1–C4), D (D1–D4), E (E1–E9 where lint-applicable; E1/E2/E6/E9 are discipline rules with limited lint surface — implement detectable cases like "non-orchestrator writer modified passport" via post-hoc passport-diff analysis), F (F1–F4).
- Two CLI modes: `--mode proposal` (validates a proposal-state entry) and `--mode persisted` (validates a persisted-state entry).
- `--passport-path <path>` and `--output-dir <path>` for cross-file rule checks.
